...
pp_BIO_v2 <- function(chlpix) {
    ...
    E0dir = array(NaN,c(23,nstep,301))
    E0dif = array(NaN,c(23,nstep,301))
    E0d = array(NaN,c(23,nstep,301))
    ...
    for (it in xhr) # loop on the time of day
    { 
        # !!!!!! WE ENTER THE NEXT LOOP ONLY IF ZENDR IS POSTIVE (E.G. DAYLIGHT!)
        if (zendR[it] <= 90)
        {
            
            for (iz in 2:nstep) # loop on depth, w start at the second step as the first one is Eqdw
            {
                ...
                E0d[it,iz,] = E0dir[it,iz,] + E0dif[it,iz,]
